    ...
    private List<LintResult> lintNode(String path, Rule rule) throws InvalidRulesetException {
        ...
        for (RuleThen then : rule.then) {
            ...
            for (LintTarget target : lintTargets) {
                ...
                Map<String,Object> options = then.getFunctionOptions();
                Object notMatch = null;
                if (options != null) {
                    notMatch = options.get("notMatch");
                }

                placeholders.put("description", rule.getDescription());
                placeholders.put("path", targetPath);
                placeholders.put("value", target.value.toString());
                placeholders.put("property",parentPath.get(parentPath.size()-1));
                placeholders.put("error", target.value.toString()+" must not match the pattern " + notMatch);
